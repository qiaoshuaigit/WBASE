 ...
 * @modify om
 */
public class NHTDB2Dialect extends Dialect {

    public NHTDB2Dialect() {
        super();
    ...
    private static boolean hasDistinct(String sql) {
        return sql.toLowerCase().indexOf("select ")>=0;//distinct
    }
...
//        System.out.println( new DB2Dialect().getLimitString("/*foo*/ select * from foos foo order by foo.bar, foo.baz", true) );
        System.out.println( new NHTDB2Dialect().getLimitString("select * from FUNCTION_INFO f order by f.FUNC_CODE", true) );
    }
