	...
	try{
		...
		if (typeof(_dropdown_succeed)!="undefined" && !isTrue(_dropdown_succeed)){
			...
			if (dropDownType=="dynamic"){
				...
				else if (typeof(datasetDropDownTree)!="undefined") _dropdown_dataset=datasetDropDownTree;
			}
			initDocument();
				...
				if (!_dropdown_box.cached){
						...
						case "date":{
							...
							_editor.onkeydown = function() {
								if(13 == window.event.keyCode) {
									var inputDateTime = parseDate(_editor.value);
									if(""!=_editor.value && "NaN"!=inputDateTime){
										setCalendarDate(inputDateTime);
										event.returnValue = false;
										changeCalendarTime(inputDateTime.getHours(),inputDateTime.getMinutes(),inputDateTime.getSeconds());
										dropDownSelected();
										nextObj = getNextTabElement(_editor);
										if (nextObj) nextObj.focus();
									}else{
										void _calendar_today_onclick();
									}
								}
								else if(27 == window.event.keyCode) {
									hideDropDown();
								}
							}
							break;
					...
					switch (dropdown.type){
						...
						case "custom":{
							...
							if(dropdown.dropdown_window.editor_value) {
								dropdown.dropdown_window.editor_value.focus();
							}
							break;
						}
...
function _standard_dropdown_keyDown(keycode){
	switch(keycode){
		...
		case 33:{
			if (_dropdown_dataset){
				_select_count_ ++;
				if(_dropdown_parentbox.dropDown.type == "dynamic") _dropdown_div.focus();
...
function dropDownSelected(noHide){
	var record;
	switch (_dropdown_parentbox.dropDown.type){
		case "list":noHide = false;
		case "dataset":noHide = false;
		case "dynamic":{
		...
		case "date":{
			...
			_tmp_dataset_date.setValue("value", new Date(_calendarControl.year, _calendarControl.month, _calendarControl.day,_calendarControl.hours,_calendarControl.minutes,_calendarControl.seconds));
	...
	if (record){
		...
		if(!noHide || 1 == _dropdown_dataset.length) {
			hideDropDown();
			_select_count_ = 0;
		}
		if(_select_count_ > 1) {
...
function createCalendar(parent_element){
	...
	function calendar(){
		...
		this.todayHours=time.getHours();
		this.todayMinutes=time.getMinutes();
		this.todaySeconds=time.getSeconds();
	...
	tmpHTML+="<TR class=\"editor\"><TD align=left>&nbsp;时间：&nbsp;";
	tmpHTML+="<select extra=select id=\"_calender_hours\" name=\"_calender_hours\" >";
	for(var i=0;i<=23;i++){
		var hh;
		if(i<=9){
			hh = "0"+i;
		...
		tmpHTML+="<option id=\"_calender_hours_id_"+hh+"\" value=\""+hh+"\"" ;
		if(_calendarControl.todayHours==i){
			tmpHTML+=" selected"
		}
		tmpHTML+=">"+hh+"</option>";	
	}
	...
	tmpHTML+="<select extra=select id=\"_calender_minutes\" name=\"_calender_minutes\">";
	for(var i=0;i<=59;i++){
		var mm;
		if(i<=9){
			mm = "0"+i;
		...
		tmpHTML+="<option id=\"_calender_minutes_id_"+mm+"\" value=\""+mm+"\"";
		if(_calendarControl.todayMinutes==i){
			tmpHTML+=" selected"
		}
		tmpHTML+=">"+mm+"</option>";	
	}
	...
	tmpHTML+="<select extra=select id=\"_calender_seconds\" name=\"_calender_seconds\">";
	for(var i=0;i<=59;i++){
		var ss;
		if(i<=9){
			ss = "0"+i;
		...
		tmpHTML+="<option id=\"_calender_seconds_id_"+ss+"\" value=\""+ss+"\"";
		if(_calendarControl.todaySeconds==i){
			tmpHTML+=" selected"
		}		
		tmpHTML+=">"+ss+"</option>";
	}
	...
	if(_dropdown_box && _dropdown_box.editor && _dropdown_box.editor.value != "") {
		var lastSelectValue = _dropdown_box.editor.value;
		var lastValues = lastSelectValue.split(" ");
		if(lastValues.length>0){
			var lastDates = lastValues[0].split("-");
			if(3==lastDates.length){
				var lastSelectDate = new Date(lastDates[0], lastDates[1] - 1, lastDates[2]);
				setCalendarDate(lastSelectDate);
			}
			if(2==lastValues.length){
				var lastTimes = lastValues[1].split(":");
				if(3==lastTimes.length){
					var lastSelectDate = new Date(lastDates[0], lastDates[1] - 1, lastDates[2],lastTimes[0], lastTimes[1], lastTimes[2]);
					setCalendarTime(lastSelectDate);
				}
			}
		}
	}
		...
		changeCalendarTime(_calendarControl.todayHours,_calendarControl.todayMinutes,_calendarControl.todaySeconds);
	}
}

//add th
function setCalendarTime(date){
	changeCalendarTime(date.getHours(),date.getMinutes(),date.getSeconds());
} 

//add th
function changeCalendarTime(hours,minutes,seconds){
	try{
		var _hours = document.getElementById("_calender_hours").value;
		var _minutes = document.getElementById("_calender_minutes").value;
		var _seconds = document.getElementById("_calender_seconds").value;
		
		if(hours){
			_calendarControl.hours = hours<=9 ? "0"+hours : hours ;
		}else{
			_calendarControl.hours = _hours
		}
		if(minutes){
			_calendarControl.minutes = minutes<=9 ? "0"+minutes : minutes;
		}else{
			_calendarControl.minutes = _minutes;
		}
		if(seconds){
			_calendarControl.seconds = seconds<=9 ? "0"+seconds : seconds;
		}else{
			_calendarControl.seconds = _seconds;
		}
		
		document.getElementById("_calender_hours_id_"+_calendarControl.hours.toString()).selected=true;
		document.getElementById("_calender_minutes_id_"+_calendarControl.minutes).selected =true;
		document.getElementById("_calender_seconds_id_"+_calendarControl.seconds).selected =true;
	}catch(e){}
}
...
function changeCalendarDate(year, month, day){
	...
	var lastDayIndex = curNumdays + _calendarControl.startday - 1 + 7;
	var activeCellIndex = _calendarControl.day + _calendarControl.startday - 1 + 7;
	if(activeCellIndex > lastDayIndex) {
		activeCellIndex = lastDayIndex;
	}
	setCalendarActiveCell(calendarData.cells[activeCellIndex]);
}
