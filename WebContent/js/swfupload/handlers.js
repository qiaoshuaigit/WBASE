...
function fileQueueError(file, errorCode, message) {
	try {
		...
		case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
			imageName = "<font color='red'>文件为空</font>";
			break;
...
function fileQueued(file){
	addReadyFileInfo(file.id,file.name,"上传队列");
}
...
function addReadyFileInfo(fileid,fileName,message,status){
	...
	col4.innerHTML = "<a href='javascript:deleteFile(\""+fileid+"\")'><u><font color=#8000FF>删除</font></u></a>";
	col1.style.width="80";
...
function uploadComplete(file) {
	try {
		...
		if (this.getStats().files_queued > 0) {
			...
			//progress.setStatus("<font color='red'>......已上传完毕!</b></font>");
			progress.toggleCancel(false);
		}
	} catch (ex) {
		this.debug(ex);
