...
import com.huateng.stl4j.bean.BranchAnyFieldsBean;
 ...
 * @date 
 */
public class BranchAnyFieldsListGetter extends BaseGetter {
	private static Logger logger = Logger.getLogger(BranchAnyFieldsListGetter.class);
	public Result call() throws AppException {
		try {
			...
			hql.append("select po.id, po.brhNo, po.brhName,po.cityCd ");
			hql.append("from BranchInfo po ");
			hql.append("where 1=1 and po.status='1' ");
			...
			List<BranchAnyFieldsBean> list = new ArrayList<BranchAnyFieldsBean>();
			Iterator it = queryResult.getQueryResult().iterator();
			while (it.hasNext()) {
	        	Object[] obj = (Object[]) it.next();
	        	BranchAnyFieldsBean bean = new BranchAnyFieldsBean();
	        	bean.setId((Integer) obj[0]);
	        	bean.setBrhNo(OmUtils.trim((String) obj[1]));
	        	bean.setBrhName(OmUtils.trim((String) obj[2]));
	        	bean.setOrgan4Cup(DataDicService.getInstance().mapInToOut2(100101, OmUtils.trim((String) obj[3])));
	        	list.add(bean);
			}
